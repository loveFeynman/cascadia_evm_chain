...
```bash
cascadiad start --json-rpc.enable=true --json-rpc.api="eth,web3,net"
```

## Key Management

To run a node with the same key every time: replace `cascadiad keys add $KEY` in `./init.sh` with:

```bash
echo "your mnemonic here" | cascadiad keys add $KEY --recover
```
...
```bash
cascadiad config keyring-backend file
```
...
::: danger
If you are running a **validator node**, always be careful when doing `cascadiad unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
...
```bash
rm $HOME/.cascadiad/config/addrbook.json $HOME/.cascadiad/config/genesis.json
...
```shell
rate(cascadiad_transactions_processed[1m])
```
